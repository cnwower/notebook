...
var outer = {
    "case0": {
        ...
        "case0_1": {
            "title": "Markdown语法解析",
            "content": "0. 点击右下角箭头图标展开**Markdown**解析窗口；\n" +
                "1. 支持实时解析，请在该窗口内输入内容。"
        }
    },
}
...
$(function () {
    ...
    $('.item:eq(0)').find('h5').text(item0Content.title).end().find('p').text(item0Content.content);
    $('.item:eq(1)').find('h5').text(item1Content.title).end().find('p').text(item1Content.content);
})